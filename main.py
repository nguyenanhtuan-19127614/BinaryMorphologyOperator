...
def operator(in_file, out_file, mor_op, wait_key_time=0):
    ...
    if mor_op == 'dilate':
        ...
        img_closing_manual = binary.opening(img, kernel)
        ...
        HoM_kernel = np.array((
            ...
            [-1, 1, 1],
            [-1, 1, 0]), dtype="int")
        img_HoM = cv2.morphologyEx(img, cv2.MORPH_HITMISS, HoM_kernel)
        cv2.imshow('OpenCV Hit or Miss image', img_HoM)
        cv2.waitKey(wait_key_time)

        img_HoM_manual = binary.Hit_or_Miss(img)
        cv2.imshow('manual Hit or Miss image', img_HoM_manual)
        cv2.waitKey(wait_key_time)

        img_out = img_HoM_manual

    elif mor_op == 'Boundary_Extraction':
        img_BE_manual = binary.BoundaryExtraction(img,kernel)
        cv2.imshow('manual Boundary Extraction image', img_BE_manual)
        cv2.waitKey(wait_key_time)
        img_out = img_BE_manual

    elif mor_op == 'Thinning':
        img_Thinning= cv2.ximgproc.thinning(img)
        cv2.imshow('OpenCV Hit or Miss image', img_Thinning)
        cv2.waitKey(wait_key_time)
    ...
    if img_out is not None:
        cv2.imwrite(out_file, 255*img_out)
